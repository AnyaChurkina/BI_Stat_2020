...
Differential expression based on experiment result

```{r message=FALSE, warning=FALSE}
res <- model.matrix(~ Data_withoutNA$result)
rfit <- lmFit(mice_transpose, res)
rfit <- eBayes(rfit)
result_stats <- topTable(rfit, number = dim(mice_transpose)[1], adjust.method = "BH")
df_limma_result <- data_frame(log2foldchange = result_stats$logFC, 
                       qval = result_stats$adj.P.Val,
                       gene_symbol = rownames(result_stats),
                       is_significant = qval < 0.05) 
```
...
ggplot(df_limma_result, aes(x = log2foldchange, y = -log10(qval), color = is_significant, label = gene_symbol)) +
  geom_point(alpha = 0.7, size = 1) +
  scale_color_manual(values = color_type, name = "Significantly differentially expressed") +
  geom_vline(xintercept=0, col="gray") +
        geom_text_repel() +
  ...
  labs(x = expression(log[2]~"(fold change)"),
       y = expression(-log[10]~"(q-value)"),
       title = "Differential expression in Result subgroup") +
  theme(plot.title = element_text(size=12, face="bold", hjust = 0.5)) +
        xlim(-1.5, 1.5) +
  theme(legend.position = "bottom",
        legend.box.background = element_rect(linetype = 0.5, size = 2))
