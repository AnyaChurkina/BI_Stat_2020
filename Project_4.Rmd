...
## Сonvert the type of variables to the correct type
...
```
      ...
      The age of people who presence residual disease is higher than people without residual disease
...
```
    ...
    To visualize the statistical differences in survival between groups I use ggsurvplot:
    
```{r}
ggsurvplot(km_fit_resid_disease, data = ovarian, xlab = "Time in days",
   ylab = "Survival", pval=TRUE, conf.int = TRUE, ggtheme = theme_bw(), 
   legend.labs = c("No", "Yes"),
   legend = "right", linetype = 1, 
   title = "Kaplan Meier survival curve based on residual disease present") 
```
  
    Observed that the survival rate of people with residual disease is lower than without them
    The obtained p-value, I'll check it with log-rank test in the future (see Task 2)
...
autoplot(km_fit_trt) + 
  ggtitle("Kaplan Meier survival curve based on treatment group") +
  xlab("Time (day)") + ylab("Survival") +
  theme(plot.title = element_text(size=14, face="bold", hjust = 0.7))
```

    And ggsurvplot with p-value:
    
```{r}
ggsurvplot(km_fit_trt, data = ovarian, xlab = "Time in days",
   ylab = "Survival", pval=TRUE, conf.int = TRUE, ggtheme = theme_bw(), 
   legend.labs = c("1", "2"),
   legend = "right", linetype = 1, 
   title = "Kaplan Meier survival curve based on treatment group") 
```
    
    It's noticeably that the survival rate of people in first treatment group is lower than in second group
...
```{r}
ggsurvplot(km_fit_ecog, data = ovarian, xlab = "Time in days",
   ylab = "Survival", pval=TRUE, conf.int = TRUE, ggtheme = theme_bw(), 
   legend.labs = c("1", "2"),
   legend = "right", linetype = 1, 
   title = "Kaplan Meier survival curve based on ECOG performance status") 
```
    
    It's noticeably that the survival rate of people with ECOG performance status-2 is lower than ECOG performance status-1
...
```{r}
km_age_fit <- survfit(Surv(futime, fustat) ~ age_60 , 
                                data=ovarian_by_age)
summary(km_fit_ecog)

ggsurvplot(km_age_fit, data = ovarian_by_age, xlab = "Time in days",
   ylab = "Survival", pval=TRUE, conf.int = TRUE, ggtheme = theme_bw(), 
   legend.labs = c("Over 60", "Under 60"),
   legend = "right", linetype = 1, 
   title = "Kaplan Meier survival curve based on age(60 years)")
```

    It can be noted that the survival rate of patients under the age of 60 is higher than that of patients over 60 (p-value < 0.05)
...
```
    ...
    It can be noted that the survival rate for people under 60 years is higher in treatment group 2 than in group 1.
...
```
    ...
    Observed that in all groups of age (under and over 60) survival rate with ECOG performance status=1 higher than ECOG performance status=2
...
## Between age (60 years)

```{r}
survdiff(Surv(futime, fustat) ~ age_60, data = ovarian_by_age)
logrank_test(Surv(futime, fustat) ~ age_60, data = ovarian_by_age)
```

    Conclusion: survival between group over and under 60 years significantly different:
        - survdiff: Chisq= 17.1  on 1 degrees of freedom, p-value = 4e-05
        - logrank_test: Z = -2.9778, p-value = 0.002903
